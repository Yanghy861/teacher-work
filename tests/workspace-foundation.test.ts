...
describe('workspace paths and SQLite foundation', () => {
  it('initializes the isolated directory tree and reopens without duplicate migrations', () => {
    ...
    expect(first.identity).toEqual({ workspaceId: 'workspace-test-id', schemaVersion: 2 })
    expect(getAppliedMigrationVersions(first.database.raw)).toEqual([1, 2])
    first.close()

    const second = initializeWorkspace(root, installDirectory, { idFactory: () => 'should-not-replace-id' })
    expect(second.identity).toEqual({ workspaceId: 'workspace-test-id', schemaVersion: 2 })
    expect(getAppliedMigrationVersions(second.database.raw)).toEqual([1, 2])
  ...
  it('rolls back a failed migration and does not advance the schema version', () => {
    ...
    expect(reopened.identity.schemaVersion).toBe(2)
    expect(getAppliedMigrationVersions(reopened.database.raw)).toEqual([1, 2])
