...
- 运行命令：
  ...
  node spikes/document-parser/run-spike.mjs `
    ...
    --output D:\teacher_work\spikes\document-parser\results\officeparser-7.5.1.json
  ```

- 机器报告：`spikes/document-parser/results/officeparser-7.5.1.json`（被 `.gitignore` 忽略，不提交样本或报告）。报告只保存匿名样本 ID、格式、大小、状态、位置计数、耗时、RSS 和固定诊断信号，不保存文件名、路径或正文。
...
- 采用候选：`officeparser@7.5.1`。本地 package manifest 标注 MIT、Node `>=18.0.0`，源码仓库为 `https://github.com/harshankur/officeParser`；npm 包声明支持 DOCX、PPTX、XLSX、PDF。该版本能把非 ZIP、截断 ZIP 与缺必需 OOXML 部件作为结构化错误交给自有 Adapter 映射为 `parse_failed`。
- 依赖与打包风险：上游仍声明 `pdfjs-dist@6.1.200`，本项目用精确 override 固定到修复 GHSA-hq66-cqwq-w95j 的 `6.2.108`；`npm audit` 为 0 high/0 critical，恶意 PDF JavaScript canary 未执行。完整处置记录在 `spikes/document-parser/security-disposition.json`。本次不启用 OCR 或 optional `@napi-rs/canvas`，正式打包仍需复核资源、体积与 worker 版本一致性。
- Electron/Windows smoke：`run-electron-smoke.mjs` 用 Electron `43.4.1` / Node `24.18.1` 主进程加载同一 Adapter 与 `pdfjs-dist@6.2.108`，分别解析 PPTX、文本 PDF、XLSX，3 项均 `indexed`（151/303/124 chunks）并以退出码 0 正常结束。

### 真实结果

| 格式 | 样本 | indexed | no_text | parse_failed | text chars | chunks | elapsed |
|---|---:|---:|---:|---:|---:|---:|---:|
| PPTX | 10 | 10 | 0 | 0 | 15,633 | 1,408 | 1,515.97 ms |
| DOCX | 14 | 14 | 0 | 0 | 60,352 | 2,941 | 623.78 ms |
| PDF | 14 | 9 | 5 | 0 | 140,017 | 7,992 | 2,444.03 ms |
| XLSX | 2 | 2 | 0 | 0 | 6,879 | 456 | 27.01 ms |
| 合计 | 40 | 35 | 5 | 0 | 222,881 | 12,797 | 4,610.79 ms |

- 峰值 RSS：467,812,352 bytes；最慢单文件为 `sample-033`，约 1,386.30 ms、6,374 chunks。
...
- 结构/降级信号：累计 CJK 字符 99,271、数学符号 736；AST 节点包含 222 slide、290 page、3 sheet、584 cell、7 table、325 image、263 heading。固定诊断查询命中：`有理数` 245、`一元二次` 143、`函数` 42、`几何` 26、`圆` 72；`AMC8`、`P16`、`|x|`、`∠ABC`、`△ABC`、`x²` 在固定诊断信号中没有命中，交由 T05 真实语料 benchmark 验证。
- 警告分类：5 条 `EMPTY_CHUNK_GENERATED`（与无文本扫描 PDF 对应）、3 条 `WHITESPACE_NODE_SKIPPED`；40 份正常样本没有 `parse_failed`。另外三个损坏 OOXML 夹具分别以 `ZIP_NO_ENTRIES_FOUND`、`ZIP_TRUNCATED`、`REQUIRED_PART_MISSING` 正确映射为 `parse_failed`。

### Spike A 决策

- **采用为当前候选 Adapter：** `officeparser@7.5.1` 可在真实 40 份样本上完成四种格式解析，并能为 PPTX/PDF/XLSX 提供可用位置；生产层只能依赖自有契约，不得保存第三方 AST。
...
- 语料直接复用 Spike A 的 40 份真实样本和 `officeparser@7.5.1` Adapter；重新提取得到 12,797 个非空 chunk，超过 T05 要求的 10,000，未加入合成教学正文。
- 运行命令：
  ...
  node spikes/chinese-search/run-benchmark.mjs `
    ...
    --output D:\teacher_work\spikes\chinese-search\results\officeparser-7.5.1.json
  ```
...
- 机器报告：`spikes/chinese-search/results/officeparser-7.5.1.json`（被 `.gitignore` 忽略）。报告不保存正文、原始文件名或路径；原文只在临时 SQLite 文件中用于建索引，报告完成后删除。

### 实测对比

- 索引构建：140.726 ms；临时 SQLite 体积 5,287,936 bytes；规范化 Token 数 41,359；Normalizer 的大小写、全角/半角、平方/立方、常见数学符号等价检查 6/6 通过，展示原文未被改写。

| 变体 | 冷首查 ms | 热 P50 ms | 热 P95 ms | 固定正例命中 | 负例误召片段 |
|---|---:|---:|---:|---:|---:|
| raw FTS5 trigram | 0.778 | 0.036 | 0.166 | 2/6 | 0 |
| FTS5 trigram + SearchNormalizer | 0.976 | 0.044 | 0.179 | 3/6 | 0 |
| 应用层 TokenExtractor | 0.883 | 0.013 | 0.482 | 6/6 | 30 |
| 短词 fallback（仅 ≤2 字符） | 0.090 | 0.003 | 0.139 | 3/6 | 0 |
| 标题/文件名精确匹配 | 0.156 | 0.033 | 0.048 | 0/6 | 0 |
...
- 权威刷新探针：`node spikes/office-watcher/run-reconciliation-probe.mjs`；实际文件读写结果为 `status=passed`，在没有任何 watcher 事件的条件下，`startup`、`focus-return`、`reopen`、`manual-refresh` 均能发现新 Hash；10 项断言全部通过，4 个新 Hash 各产生一次重建决定，2 个并发触发被合并。
- 产品一致性边界调整为：刷新核对保证正确性，watcher 只是可选加速器。已有三格式真实 WPS 记录证明代表性普通保存能形成可读最终文件；自动恢复、大文件保存和保存中退出不再属于 T06/T08 的强制矩阵。
...
| watcher 完全漏报后的刷新核对 | 自动化真实文件探针；不注入 watcher 事件 | 已验证；启动、焦点返回、重新打开或手动刷新均可恢复一致 |
| 自动恢复式保存 | 不作为 V1 实时事件承诺 | 不阻塞；最终文件在下一次刷新核对时按稳定性与 Hash 处理 |
| 大文件保存 | 不规定固定容量阈值；Hash/解析仍必须在后台 Worker | 不阻塞 T06；响应性在 T18/T24/T32 用后台任务与代表性大文件验证 |
| 保存进行中退出 | 不依赖退出瞬间的 watcher 事件 | 不阻塞；不可读时保留待核对并有界重试，下次启动/刷新继续 |

上述三个 WPS 内部边界不再承担 V1 正确性：无论它们产生什么事件，只要最终文件可读，权威刷新核对就会在启动、焦点返回、重新打开或手动刷新时比较 Hash。未来若产品要承诺“无需返回工作台即可实时更新”或明确的大文件性能上限，再补专项实验，不反向阻塞当前 V1。
...
- **权威路径：** 后续 FileChangeService 必须实现启动后台核对、工作台焦点返回、重新打开和手动刷新；统一经过 stable/readable/hash/coalescing，并在 watcher 完全漏报时仍正确。
- **可选加速候选：** 正式 Main/Worker watcher 可从 Chokidar `4.0.3` 开始，保留 `fs.watch` 作为替代候选和回归对照；watcher 只标 dirty，不能成为唯一变化来源。
- **不接入正式索引器：** 本 Spike 只证明刷新核对、可选事件归一化和任务调度边界，不创建索引、不读取 Renderer 文件系统，也不改变原始教学资料。
- **已知风险：** 如果老师在工作台之外直接修改文件、工作台持续在后台且 watcher 又漏报，搜索结果会暂时陈旧；下一次工作区启动、焦点返回、重新打开或手动刷新后恢复。UI 必须展示索引更新时间/更新中状态并提供“刷新资料”。
