...
| V1532-D 最终回归 | IN_PROGRESS | 已按用户反馈将 Renderer 原生确认/输入框统一替换为应用内弹窗，覆盖从本课移除资料、AI 修改、快速建课及素材库目录维护；仍需产品负责人实际走查两棵树、目录维护和资料流转后，再决定是否创建 V1.5.3 最终通过标签；当前未创建新 checkpoint |
