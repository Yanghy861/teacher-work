# 教师工作台 V1 / V1.1 / V1.2 / V1.3 · Git 版本控制协议
...
3. 普通里程碑运行相关测试、typecheck、lint；V1/V1.1 历史闸门按原协议执行；V13-05 运行全量测试、typecheck、lint、production build、`git diff --check` 和代表性本地 Windows 流程；V1.3 不运行 portable/installer packaging；
...
7. 验收齐全且状态为 `DONE` 时，历史 V1 使用 `lean(LXX): <简短名称>`，V1.1 使用 `v1.1(V11-XX): <简短名称>`，V1.2 使用 `v1.2(V12-XX): <简短名称>`，V1.3 使用 `v1.3(V13-XX): <简短名称>`；真实阻塞使用对应任务号提交 `blocked(<任务号>): <原因>`；
...
上述审核链已全部完成，只用于历史追溯。V1.1 与 V1.2 的最终验收也已完成。V1.3 不复制多阶段审核流程：V13-01–V13-04 正常完成和提交；V13-05 是唯一最终验收点，形成 `docs/v1.3-acceptance.md`，在自动质量门、代表性本地 Windows 流程和产品负责人体验确认均完成后创建 `checkpoint-V1.3-pass`。
