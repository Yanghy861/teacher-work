...
2. **md 直接编辑（人工编辑，2026-09-03 升格为 V1.7 必做）**：阅读器对 md 文件提供编辑 + 预览切换，保存语义（第 N+1 版 vs 直接改当前版）立项时定。
3. **AI 写课件 ↔ 题库打通（2026-09-03 产品负责人澄清最终目标）**：目标**不是**老师手动从题库挑题注入参考，而是——生成时选上"参考题库"后，**AI 自动**依据描述（课次主题、年级等）检索题库中对应 tag/年级/难度的题目并自行挑选合适的写入课件；题库题目均带年级/难度/tag 结构化元数据。设计方向（两阶段生成，详见 V1.7 设计基准）：
   - 阶段 A（检索规划）：把老师生成请求 + 课次上下文 + 题库 facet 摘要（tags/年级/难度/题型及计数，`QuestionBankSummary` 已有）交给 AI，AI 输出结构化检索计划（JSON，1:1 映射 `QuestionBankSearchRequest`：text/tags/grade/difficultyMin-Max/type/目标题数）；
   - 检索执行：Main 侧用既有 `questionBankService.search` 本地确定性执行（AI 不直接碰库，无新 IPC）；
   - 阶段 B（选题生成）：候选题目全文（题干+选项+答案+解析）按 D25 预算进上下文，AI 从候选中选题写作，指令明确"不得杜撰题目、只能从候选中选"；候选块作为一项参考计入 30,000 字预算与确认弹窗；
   - 交互：参考区新增"参考题库（AI 自动选题）"开关（未安装题库时置灰引导），方案确认卡可复用为"检索计划 + 已选题目"过目/微调步（"再难一点/去掉尺规作图"式反馈重检索）——是否保留人工过目步由产品负责人定；
   - 边界：保持"老师选题目标、AI 检索执行"定位，不做自动组卷/错题本；含图题目（DeepSeek 无 vision）V1.7 处理方式立项时定。
   - 候选实现（已否决）：OpenAI 式 function calling 工具循环（多轮工具调用与流式面板/取消/超时交织复杂、难测试，两阶段确定性方案更稳）；手动挑题注入（产品负责人已明确不是目标）。
   - 沉淀侧（课件题目存回题库形成个人题区）保持为后续版本候选。
4. ~~扫描件就地提示增强~~（2026-09-02 产品负责人裁决：**不需要**，从候选中移除）。

**2026-09-03 产品负责人两项裁决补充（V1.7 设计基准输入）：**

- **学生版 / 教师版双输出生成**：题库参与感问题的解法——AI 选题生成的课件出两份：教师版（含"选自题库（tag/难度）"标注 + 答案解析，供老师回顾）与 学生版（纯题目、无标注无答案，直接印发）。实现取舍（立项时定）：教师版为一次主生成；学生版可选 ① 第二次快速生成（同步性好、成本略增）或 ② 本地规则剥离（要求生成格式把答案解析放固定区块，零成本但格式刚性）；发布/命名沿用版本链语义。
- **题库 ID 稳定性结论（代码核实，无新增工作）**：`.tqbank` 导出器已强制校验每题具备稳定 `source_uid`（缺失即抛错拒绝导出），且 `source_uid` 为主键、`source_question_id` 唯一——**"一题一 ID"从第一天即被保证**；题库为整库快照替换（`current.tqbank` 整体换文件），同题在新快照中携带同一 `source_uid`，无需任何"换索引"机制；生成侧产物中题目以文本固化进课件 md，不存运行时 ID 依赖，快照换版不影响历史课件；`content_hash` 字段已存在，可供未来沉淀侧检测"同 ID 不同内容"的边界情况。

**Git 同步（2026-09-02）：** 应产品负责人要求接入 GitHub（既有远程 `github.com/Yanghy861/teacher-work`），V1.6 全链 11 提交与 `checkpoint-V1.6-pass` 标签已推送；推送前经历史扫描确认无个人信息与密钥形态内容入库。
