...
**状态：** `DONE`（2026-09-02 产品负责人最终验收通过；含 2026-09-02 两轮真实自测反馈与当日修复）
...
## 最终验收记录（2026-09-02，产品负责人确认）

- **产品负责人最终验收结论："其他的我验收下来已经没问题了"**——V1.6 全部自动门 + 隔离冒烟 + 两轮真实自测反馈全部通过：
  - 第一轮（DeepSeek 真实使用）：流式生成、思考秒表（A1 裁决落地）、显示公式渲染修复后确认可用；参考预算确认弹窗、修改收口体验按前述反馈处理完毕；
  - 第二轮（MinerU）：产品负责人确认核心验收项"参考文档传给 AI 无感流转生成"通过（本机解析路径）；MinerU 手动流按裁决作为按需能力跳过（无 token 场景），入口可见性缺陷当日修复；
- 本轮真实自测费用：DeepSeek 约 ¥1–2（低于 ¥3 门槛）；MinerU 未使用（¥0）。
- 依据 VERSION_CONTROL 协议，在最终确认提交上创建 `checkpoint-V1.6-pass`（标签说明注明基线 `checkpoint-V1.5.6-pass`）。

## V1.7 需求记录（2026-09-02 产品负责人提出，立项待开工确认）

产品负责人新裁决（取代反馈 C 原提案中"仅应用内生成 md 可编辑"的范围）：

1. **所有 md 文件可 AI 二次编辑（V1.7 需做）**："按理说所有的 md 文件应该都是可以用 AI 进行二次编辑的吧，不一定必须是通过 AI 生成的才能改"——即 AI 修改流的"修改对象"从 D23 收口的"应用内课件版本"扩展到**任意 text/markdown 的 managed 文件**（含外部导入的 md）。V1.6 的 D23 收口（仅 ` · 第 N 版.md` 模式）在 V1.7 中放宽；AI 修改产物仍按版本语义保存，不覆盖老师原件（安全边界不变）。
2. **md 直接编辑（人工编辑，V1.7 候选）**：反馈 C 原提案——阅读器对 md 提供编辑 + 预览，保存语义（第 N+1 版 vs 直接改当前版）立项时定。
3. **扫描件就地提示增强（V1.7 候选）**：反馈 D 衍生——参考选择时对无文本文件就地提示一键 MinerU 增强。
