...
3. **AI 写课件 ↔ 题库打通（2026-09-02 新需求，方向探讨中）**：题库现状 = `.tqbank` 只读快照（结构化题目：题干/选项/答案/解析/难度/标签/图片），已有单题 copyToLesson 导出 md 的能力。建议两个方向（详见立项设计基准时确认）：
   - **生成侧（题库 → 课件）**：draft-panel 参考区新增"从题库选题"入口（复用题库既有搜索/筛选/预览 UI），勾选题目直接注入生成请求（draft contract 扩展 `bankQuestions` 载荷字段，Main 侧题目文本渲染入 buildContext，与文件参考共用 30,000 字预算与确认弹窗；含图题目注明"含图未纳入"或走既有导出路径）；不新增 IPC 通道。
   - **沉淀侧（课件 → 题库）**：需先把题库从只读快照扩展出"个人题目区"（新表，与导入快照并列、同样可搜索/可被生成引用），课件 md 中题目块一键存入；工作量大于生成侧，建议列为 V1.7 第二步或 V1.8。
   - 边界提醒：保持"老师选题、AI 排版讲解"定位，不演变为自动组卷/错题本（V1.6 版 AGENTS 曾明令禁止，V1.7 由产品负责人在此重新定界）。
