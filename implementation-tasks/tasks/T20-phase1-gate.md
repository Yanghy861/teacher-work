...
- 场景必须包含：张三一对一、两个不连续阶段、多节课、素材 PPT 加入 A/B 班、PowerPoint/WPS 修改 A 班、返回工作台或手动刷新后识别变化、副本完全隔离、删除课次后恢复；
- 增加故障注入：复制中断、数据库回滚、external missing/root 重定位、非法路径、watcher 漏报，以及 watcher/刷新重复触发；
