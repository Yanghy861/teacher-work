  ...
  async createBackup(destinationPath: string): Promise<BackupResult> {
    return this.activityGate.pause(async () => {
      try {
        ...
        const target = validateBackupDestination(destinationPath, this.workspace.paths.root)
...
function validateBackupDestination(destinationPath: string, workspaceRoot: string): string {
...
function validateManifest(manifest: BackupManifest, maxFiles: number, maxTotalBytes: number): void {
  if (manifest.manifestVersion !== BACKUP_MANIFEST_VERSION || manifest.backupVersion !== BACKUP_MANIFEST_VERSION) {
  ...
  if (!Number.isInteger(manifest.fileCount) || manifest.fileCount !== manifest.files.length || manifest.fileCount > maxFiles) {
  ...
  for (const file of manifest.files) {
    ...
    if (file.originalName.trim() === '' || !Number.isSafeInteger(file.sizeBytes) || file.sizeBytes < 0 ||
      !Number.isFinite(file.mtimeMs) || !Number.isInteger(file.mode)) {
...
function validateRestoredDatabase(database: SqliteDatabase, manifest: BackupManifest): void {
  ...
  if (parsedSchemaVersion > getSchemaVersion(database)) {
    throw new BackupRestoreError('RESTORE_VALIDATION_FAILED', '恢复的 schema 版本高于当前应用支持版本。')
  }
}
