...
export default function ManagedFilesPanel({ compact = false, heading = '素材库', lessonId, lessonLabel }: ManagedFilesPanelProps): React.JSX.Element {
  ...
  function createFolder(): void { const name = window.prompt('新建素材文件夹名称')?.trim(); if (name) void runAction(() => window.teacherWorkbench.materialLibrary.createFolder({ parentId: activeFolderId, name }), '文件夹已创建。') }
  ...
  return <div className={`managed-files-panel${compact ? ' is-compact' : ''}`} aria-live="polite">
    ...
    <section className="workspace-card"><div className="card-heading"><div><p className="section-kicker">资料管理</p><h2>{heading}</h2></div><div className="file-toolbar"><button className="secondary-button" type="button" onClick={() => void reload()} disabled={busy}>刷新</button><button className="secondary-button" type="button" onClick={createFolder} disabled={busy}>新建文件夹</button><button className="primary-button" type="button" onClick={importFile} disabled={busy}>导入资料</button></div></div>
...
function FolderBranch({ folder, folders, fileFolder, view, onSelect, onRename, onDelete }: { readonly folder: MaterialFolder; readonly folders: readonly MaterialFolder[]; readonly fileFolder: ReadonlyMap<string, string | null>; readonly view: LibraryView; readonly onSelect: (view: LibraryView) => void; readonly onRename: (id: string, name: string) => void; readonly onDelete: (id: string) => void }): React.JSX.Element {
  const children = folders.filter((item) => item.parentId === folder.id); const count = [...fileFolder.values()].filter((id) => id === folder.id).length
  return <div className="material-folder-branch"><div className={`material-library-tree-row${view === `folder:${folder.id}` ? ' is-selected' : ''}`}><button className="material-folder-select" type="button" onClick={() => onSelect(`folder:${folder.id}`)}><span className="material-library-tree-icon">▸</span><span title={folder.name}>{folder.name}</span><small>{count}</small></button><button className="folder-more" type="button" title="文件夹操作" onClick={() => { const action = window.prompt('输入 rename 重命名，或 delete 删除空文件夹')?.trim(); if (action === 'rename') { const name = window.prompt('新的文件夹名称', folder.name)?.trim(); if (name) onRename(folder.id, name) } else if (action === 'delete') onDelete(folder.id) }}>⋯</button></div>{children.map((child) => <FolderBranch key={child.id} folder={child} folders={folders} fileFolder={fileFolder} view={view} onSelect={onSelect} onRename={onRename} onDelete={onDelete} />)}</div>
}
